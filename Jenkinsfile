            ...
            stage ("8.3) Deploy new container to DL6") {
              steps {
                ...
                sh """#!/bin/bash
                  ...
                  -l "SERVICE_TAGS=trilogy.expose-v2,trilogy.redirecthttp,trilogy.cert=internal_default,trilogy.https,trilogy.internal,trilogy.endpoint=${STAGE}-${ENDPOINT},deploy.date=${DEPLOY_DATE},git.hash=${GIT_HASH},jenkins.build=${BUILD_NUMBER},jenkins.job=${JOB_NAME}" \
